...
agent + role: admin/write/read). Rate limiting, locked-down CORS, security
headers, body-size cap, and an audit log that never records secrets.
"""

# NOTE: no `from __future__ import annotations` here. slowapi's @limiter.limit
# wrapper carries slowapi's __globals__, so FastAPI cannot resolve stringified
# annotations like `SaveIn` on decorated routes and misreads the body as a
# query param (422). Runtime PEP 604 unions require Python >= 3.10 (already
# our floor).
import asyncio
...
from brain.security import audit, auth_configured, key_fingerprint, set_client_ip, verify_key


def _rate_key(request: Request) -> str:
    """Rate-limit key: API-key fingerprint when a bearer token is presented,
    client IP otherwise. Behind a reverse proxy all traffic shares the proxy
    IP, so IP-only keying would throttle every agent together."""
    auth = request.headers.get("authorization", "")
    if auth.lower().startswith("bearer "):
        token = auth[7:].strip()
        # Only valid keys get their own bucket — otherwise an attacker could
        # rotate random tokens to dodge the limit while guessing credentials.
        if token and verify_key(token):
            return key_fingerprint(token)
    return get_remote_address(request)


limiter = Limiter(key_func=_rate_key, default_limits=[config.rate_limit])
...
async def require_agent(
    ...
    set_client_ip(ip)  # every audit record for this request carries the IP
    token = None
    if authorization and authorization.lower().startswith("bearer "):
        token = authorization[7:].strip()
    identity = verify_key(token)
    if not identity:
        audit("auth_failed", ip=ip, path=request.url.path)
        raise HTTPException(status_code=401, detail="Invalid API key")
    return identity  # (agent, role)


_ROLE_RANK = {"read": 0, "write": 1, "admin": 2}


def _min_role(min_role: str):
    """Dependency factory: yields the agent name if the key's role covers
    `min_role`, else 403. read < write < admin."""

    async def dep(identity: tuple[str, str] = Depends(require_agent)) -> str:
        agent, role = identity
...
require_read = _min_role("read")
require_write = _min_role("write")
require_admin = _min_role("admin")
...
@app.get("/projects")
async def projects(agent: str = Depends(require_read)):
    return {"projects": brain.projects()}


@app.get("/stats")
async def stats(project: str | None = None, agent: str = Depends(require_read)):
    return brain.stats(project=project)


@app.post("/save")
@limiter.limit(config.rate_limit)
async def save(request: Request, body: SaveIn, agent: str = Depends(require_write)):
    note = brain.save(
...
@app.post("/ingest")
@limiter.limit(config.rate_limit)
async def ingest(request: Request, body: IngestIn, agent: str = Depends(require_write)):
    note = brain.ingest(
        text=body.text,
        title=body.title,
        source=body.source,
...
@app.post("/search")
@limiter.limit(config.rate_limit)
async def search(request: Request, body: SearchIn, agent: str = Depends(require_read)):
    hits = brain.search(
        query=body.query,
        limit=body.limit,
        category=body.category,
...
@app.post("/recall")
@limiter.limit(config.rate_limit)
async def recall(request: Request, body: RecallIn, agent: str = Depends(require_read)):
    bundle = brain.recall(
        query=body.query,
        project=body.project,
        user=body.user,
...
@app.post("/feedback")
async def feedback(body: FeedbackIn, agent: str = Depends(require_write)):
...
@app.get("/get/{note_id}")
async def get(note_id: str, project: str | None = None, agent: str = Depends(require_read)):
...
async def related(
    note_id: str, project: str | None = None, limit: int = 8, agent: str = Depends(require_read)
):
    return {"related": brain.related(note_id, project=project, limit=max(1, min(limit, 50)))}


@app.get("/recent")
async def recent(n: int = 20, project: str | None = None, agent: str = Depends(require_read)):
...
async def activity(
    ...
    n: int = 20,
    project: str | None = None,
    agent: str = Depends(require_read),
):
...
# --- graph --------------------------------------------------------------
@app.get("/entities")
async def entities(project: str | None = None, agent: str = Depends(require_read)):
...
@app.get("/soul")
async def get_soul(project: str | None = None, agent: str = Depends(require_read)):
    return {"soul": brain.get_soul(project=project)}


@app.post("/soul")
async def set_soul(body: SoulIn, agent: str = Depends(require_admin)):
...
@app.post("/soul/learn")
async def learn_principle(body: PrincipleIn, agent: str = Depends(require_admin)):
    scoped = agent if body.agent_scope else None
...
@app.get("/preferences")
async def get_prefs(project: str | None = None, agent: str = Depends(require_read)):
    return {"preferences": brain.get_preferences(project=project)}


@app.post("/preferences")
async def set_pref(body: PrefIn, agent: str = Depends(require_write)):
    scoped = agent if body.agent_scope else None
...
@app.get("/ontology")
async def get_ontology(agent: str = Depends(require_read)):
    return {"taxonomy": brain.get_ontology()}


@app.post("/ontology")
async def set_ontology(body: OntologyIn, agent: str = Depends(require_admin)):
...
async def consolidate(project: str | None = None, agent: str = Depends(require_write)):
...
async def dream(project: str | None = None, agent: str = Depends(require_write)):
...
async def tick(project: str | None = None, agent: str = Depends(require_write)):
...
@app.post("/reindex")
async def reindex(project: str | None = None, agent: str = Depends(require_admin)):
...
# --- facts (bi-temporal) ------------------------------------------------
@app.post("/facts")
async def add_fact(body: FactIn, agent: str = Depends(require_write)):
    fact = brain.add_fact(
...
@app.get("/facts")
async def get_facts(
    subject: str | None = None, project: str | None = None, agent: str = Depends(require_read)
):
...
async def fact_history(
    subject: str, project: str | None = None, agent: str = Depends(require_read)
):
...
# --- core memory blocks -------------------------------------------------
@app.get("/blocks")
async def get_blocks(project: str | None = None, agent: str = Depends(require_read)):
    return {"blocks": brain.list_blocks(project=project)}


@app.post("/blocks")
async def set_block(body: BlockIn, agent: str = Depends(require_write)):
    scoped = agent if body.agent_scope else None
...
@app.get("/block/{name}")
async def get_block(name: str, project: str | None = None, agent: str = Depends(require_read)):
    return {"name": name, "block": brain.get_block(name, project=project)}


# --- graph: communities + multi-hop + aliases ---------------------------
@app.get("/communities")
async def communities(project: str | None = None, agent: str = Depends(require_read)):
...
async def entity_multihop(
    entity: str, depth: int = 2, project: str | None = None, agent: str = Depends(require_read)
):
    return {
        "entity": entity,
        "depth": depth,
...
@app.post("/alias")
async def set_alias(body: AliasIn, agent: str = Depends(require_write)):
...
@app.get("/directives")
async def get_directives(project: str | None = None, agent: str = Depends(require_read)):
    return {"directives": brain.directives(project=project)}


@app.post("/directives")
async def add_directive(body: DirectiveIn, agent: str = Depends(require_admin)):
...
@app.post("/pin")
async def pin(body: PinIn, agent: str = Depends(require_write)):
...
# --- archival -----------------------------------------------------------
@app.post("/archive")
async def archive(body: ArchiveIn, agent: str = Depends(require_write)):
...
@app.get("/doctor")
async def doctor(project: str | None = None, agent: str = Depends(require_read)):
    return brain.doctor(project=project)


@app.get("/metrics")
async def metrics(agent: str = Depends(require_read)):
    return {"metrics": brain.metrics()}


@app.get("/export")
async def export(project: str | None = None, agent: str = Depends(require_admin)):
    audit("export", agent=agent, project=project)
    return brain.export(project=project)


@app.post("/import")
async def import_bundle(body: ImportIn, agent: str = Depends(require_admin)):
...
async def sleep_cycle(project: str | None = None, agent: str = Depends(require_write)):
...
@app.post("/checkpoint")
@limiter.limit(config.rate_limit)
async def checkpoint(request: Request, body: CheckpointIn, agent: str = Depends(require_write)):
...
@app.get("/sessions")
async def sessions(project: str | None = None, agent: str = Depends(require_read)):
    return {"sessions": brain.sessions(project=project)}


@app.post("/session/close")
async def session_close(body: SessionCloseIn, agent: str = Depends(require_write)):
    res = brain.close_session(body.project, body.session)
    audit("session_close", agent=agent, project=body.project, session=body.session)
    return res


@app.delete("/delete/{note_id}")
async def delete(note_id: str, project: str | None = None, agent: str = Depends(require_admin)):
